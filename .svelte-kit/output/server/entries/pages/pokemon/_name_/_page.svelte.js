    ...
    children: ($$payload2) => {
      if (pokemon) {
        ...
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          ...
          $$payload2.out += `<div><div class="flex justify-between border border-neutral-700 p-3 rounded"><span class="capitalize">${escape_html(stat.stat.name)}:</span> <span class="font-bold">${escape_html(stat.base_stat)}</span></div></div>`;
        }
        $$payload2.out += `<!--]--></ul></div></div></div></div>`;
      } else {
        $$payload2.out += "<!--[!-->";
        $$payload2.out += `<h1>Loading</h1>`;
      }
      $$payload2.out += `<!--]-->`;
    },
    $$slots: { default: true }
